...
function thDecryptImpl(buffer, blockSize, base, add) {
    const newBuffer = Buffer.from(buffer)
    let p = 0, tp1, tp2, hf, left = buffer.length

    if ((left % blockSize) < Math.trunc(blockSize / 4))
        left -= left % blockSize
    ...
    while (left) {
        ...
        tp1 = p + blockSize - 1
        tp2 = p + blockSize - 2
        hf = Math.trunc((blockSize + (blockSize & 0x1)) / 2)

        for (let i = 0; i < hf; ++i, ++p) {
            newBuffer[tp1] = buffer[p] ^ base
            base += add
            tp1 -= 2
        }
        hf = Math.trunc(blockSize / 2)

        for (let i = 0; i < hf; ++i, ++p) {
            newBuffer[tp2] = buffer[p] ^ base
            base += add
            tp2 -= 2
        }
        left -= blockSize
...
function decrypt06(buffer, key, start = 0) {
    return th06DecryptImpl(buffer, key, start)
}

function decrypt(buffer, blockSize, base, add) {
